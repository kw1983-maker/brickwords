...
export function climbRoller(world, x, y, z, len, radius, axis, colour, group) {
  ...
  // A full AABB around a lying cylinder blocks jumps into the rung above — the
  // hull hits the underside of the box long before the feet can land on top.
  // Only the crown is solid: a thin invisible cap at the top of the roller.
  const d = radius * 2;
  const capH = 0.55;
  const capW = axis === 'x' ? len : d * 0.82;
  const capD = axis === 'x' ? d * 0.82 : len;
...
// A climb: a run of rollers stepping upward in a zig-zag. Each rung is a
// 2-stud step (Roblox's automatic ledge climb) with a short sideways hop, so a
// child can walk-and-step their way up without precision jumping. Ends on a
// small top deck with a balloon, the reward for getting up there.
export function climbTower(world, x, z, group, opts = {}) {
  const g = group || world.root;
  const rungs = opts.rungs || 9;
  const rise = opts.rise || 2.0;          // <= STEP_HEIGHT — walk up each rung
  const step = opts.step || 3.5;          // comfortable sideways hop
  ...
  // The first rung's cap top sits at pad-top + STEP_HEIGHT (4 studs) so it is a
  // single step up from the ground; later rungs rise by `rise` (2 studs each).
  let cx = x, cz = z, cy = 4 - radius;
